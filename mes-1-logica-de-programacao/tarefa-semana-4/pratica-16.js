function buscaTagEspecifica(lista, termoDeBusca) {
    let listaEspecifica = []

    for(let i = 0; i<lista.length; i++) {
        if (lista[i].tags.includes(termoDeBusca)) {
            listaEspecifica.push(lista[i])
        }
    }
...
const listaImagens = [{
  ...
  buscaTagEspecifica(listaImagens, termoDeBusca)